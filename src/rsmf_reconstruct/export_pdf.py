"""PDF Report Generator & Media Consolidator for RSMF archives.
...
def generate_pdf_report(
    ...
    """Render the reconciled timeline into a PDF report using Playwright.
    ...
    try:
        with sync_playwright() as p:
            # 1. Determine the path to the bundled browser inside the EXE
            # We assume you will pack the browser into a 'chrome-win' folder
            bundle_dir = getattr(sys, "_MEIPASS", os.path.abspath("."))
            executable_path = Path(bundle_dir) / "chrome-win64" / "chrome.exe"

            launch_kwargs = {"headless": True}
            
            # 2. If the bundled browser exists, use it (Offline Mode)
            if executable_path.exists():
                logger.info(f"Using bundled browser at: {executable_path}")
                launch_kwargs["executable_path"] = str(executable_path)
            else:
                logger.warning("Bundled browser not found. Falling back to system/default Playwright.")
